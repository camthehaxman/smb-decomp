...
/*static*/ void CalcEmitter(struct SND_EMITTER* em, f32* vol, f32* doppler, f32* xPan, f32* yPan,
  ...
  *doppler = 1.f;
  ...
  for (n = 0, li = s3dListenerRoot; li != NULL; li = li->next, ++n) {
    d.x = em->pos.x - (li->pos.x);
    d.y = em->pos.y - (li->pos.y);
    d.z = em->pos.z - (li->pos.z);
...
static START_GROUP startGroup[32];  // size: 0x400
static u8 startGroupNum;            // size: 0x1
static START_LIST startListNum[32]; // size: 0x700
static u8 startListNumnum;          // size: 0x1
static RUN_LIST runList[32];        // size: 0x300
...
u32 AddStartingEmitter(SND_EMITTER* em, f32 vol, f32 xPan, f32 yPan, f32 zPan, f32 pitch) {
  long i;         // r30
  START_LIST* sl; // r29
  ...
  if (startListNumnum == 32) {
    return FALSE;
  }

  if ((sl = startGroup[i].list) != NULL) {
